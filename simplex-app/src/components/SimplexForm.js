...
function SimplexForm() {
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      console.log (contarArtificiales());
/* 
      if (method === 'Dos Fases') {
  ...
  console.log('asi se ve el array de los operadores');
  ...
  console.log(`asi se ve las de holfura: ${contarholgura()}`);
  console.log(`asi se ve las de artificiales: ${contarArtificiales()}`);


  const convertToMatrixGranM = () =>{
  ...
  const convertToMatrixDosFases = () => {//!aqui se hace lo de la M
    const holgura = contarholgura();
    const variables1 = variables;
    const columna = variables1 + holgura + contarArtificiales() + 1;
    const filas = restrictions + 2;
